...
import numpy as np
from dataset_vqacp_lxmert_qtype import Dictionary, VQAFeatureDataset
import utils_1
from tasks.model_baseline import VQAModel
from param import args as opt
...
def get_logits(model, dataloader):
    ...
    for v, b, q, a, i in iter(dataloader):
        ...
        v = v.cuda()
        b = b.cuda()
        logits = model(v, b, list(q), None, None)
...
if __name__ == '__main__':
    ...
    eval_loader = DataLoader(eval_dset, 64, shuffle=False, num_workers=1, collate_fn=utils_1.trim_collate)

    def process(args, model, eval_loader):


        print('loading %s' % opt.checkpoint_path)
        model_data = torch.load(opt.checkpoint_path, weights_only=True)
        ...
        # with open(opt.output+'/test_%s.json' % (model_label), 'w') as f:

        with open(opt.output + "/test_results.json", 'w') as f:
            json.dump(results, f)
