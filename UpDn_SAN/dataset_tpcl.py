...
def _load_dataset(dataroot, name, dataset='cvp2', label2ans=None, ratio=1.0, qtype=''):
    """Load entries
    ...
    if name == 'train':
        for question, answer in zip(questions_new, answers_new):
            ...
            # add only questions of the same question type.
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot, img_root, ratio, qtype='', adaptive=False, dataset='cpv2'):
        ...
        self.entries = _load_dataset(dataroot, name, self.dataset, self.label2ans, ratio, qtype=self.qtype)
        self.tokenize()
        self.tensorize()
